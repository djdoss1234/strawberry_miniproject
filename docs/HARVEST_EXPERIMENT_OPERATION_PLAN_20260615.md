...
- NW 잎/줄기 가림 환경 수확
- NE 군집 딸기 수확
- 원본 SafeGrasp position/current 양방향 판독 확인 완료, 조건별 임계값 보정 및
  수확 시퀀스 통합 필요
...
1. 원본 `/gripper_service/safe_grasp`와 `/gripper_service/state` ready 확인
2. 빈 파지, 줄기 파지, 잎 접촉 조건별 데이터 수집
...
`ready=true`와 유효 `present_position/present_current`를 확인한 뒤 보정 시험을
진행한다.
...
1. 빈 파지/줄기/잎 접촉 SafeGrasp 보정 표본 수집
2. cuRobo close/verify 구간을 원본 SafeGrasp action으로 통합
