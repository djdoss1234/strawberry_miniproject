...
방향은 유지하면서 파지 직전 높이 동작을 변경했다.

```text
그리퍼 position 600으로 개방
 -> KP1보다 30mm 위쪽 높이로 수평 접근
 -> 그리퍼를 연 상태로 BASE -Z 30mm 하강
 -> KP1 부근에서 gripper close
 -> BASE -Z 40mm 추가 하강으로 줄기 분리
 -> TOOL -Z 직선 후퇴
```

이 변경은 top-down 접근으로 전환한 것이 아니다. 화이트보드 방향의 기존 수평
접근을 유지하고, 마지막 파지 구간에서만 줄기 방향으로 내려와 KP1에서 닫는다.
실기 검증 시 `OPEN_STEM_DESCENT` 로그와 잎/과실 접촉 여부를 확인한다.
