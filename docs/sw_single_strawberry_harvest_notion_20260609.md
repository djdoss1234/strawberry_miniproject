...
현재 로그에 표시되는 “파지 위치 도착”은 **그리퍼가 목표 위치까지 이동했다**는
뜻이다.
...
### 현재 로봇이 자동으로 판단할 수 있는 범위

그리퍼를 닫은 뒤 실제로 얼마나 닫혔는지 위치값을 읽어 다음처럼 판단한다.

| 로봇의 판단 | 판정 방식 |
| --- | --- |
| 그리퍼 사이에 무언가 잡혔다고 추정 | 그리퍼가 끝까지 닫히지 않고 위치값 `665` 미만에서 멈춤 |
| 아무것도 잡지 못했다고 추정 | 그리퍼가 위치값 `665` 이상까지 완전히 닫힘 |
| 판단할 수 없음 | 그리퍼 위치 센서값 읽기 실패 |

최신 SW 실행에서는 **그리퍼 위치 센서값을 읽지 못했다**. 따라서 사람이
보기에는 딸기를 잡았지만, 로봇 스스로는 잡았는지 판단하지 못한 상태다.

### 아직 실제 로그에 자동 판정되지 않는 값

| 앞으로 확인할 항목 | 확인할 내용 | 현재 상태 |
| --- | --- | --- |
...
주의할 점은 그리퍼가 중간에서 멈췄더라도 줄기가 아니라 잎을 잡았을 수 있다는
것이다. 따라서 그리퍼가 닫힌 정도만으로 최종 수확 성공을 선언하지 않고,
