...
DETACH_PITCH_DOWN_DEG  = 30.0   # 파지 후 그리퍼 TOOL-Y 피치 각도 (아래로 당기기)
DETACH_PITCH_VEL_DEG_S = 25.0   # 각속도 (빠르면 충격으로 분리 도움)
...
class CuroboPlanner(Node):
    ...
    def _execute_pitch_detach(self) -> bool:
        """파지 후 그리퍼를 TOOL-Y 기준으로 피치-다운 회전하여 줄기 분리.

        WALL_QUAT 기준: TOOL-Y = world [-1,0,0]. TOOL-Y 기준 +N° 회전 →
        TOOL-Z([0,1,0])가 world -Z 방향으로 기울어지면서 그리퍼 코가 아래를 향함.
        cuRobo 검증 없이 직접 실행 (파지 위치가 이미 벽 모델 안쪽일 수 있음).
        실패해도 호출부가 retreat을 계속 실행한다.
        """
        if not self.cli_movel.wait_for_service(timeout_sec=3.0):
            self.get_logger().warn("DETACH_PITCH: MoveLine service unavailable")
            return False
        req = MoveLine.Request()
        req.pos = [0.0, 0.0, 0.0, 0.0, float(DETACH_PITCH_DOWN_DEG), 0.0]
        req.vel = [5.0, float(DETACH_PITCH_VEL_DEG_S)]
        req.acc = [10.0, 40.0]
        req.time = 0.0
        req.radius = 0.0
        req.ref = 1   # TOOL
        ...
        self.get_logger().info(
            f"DETACH_PITCH_DOWN: TOOL-Y +{DETACH_PITCH_DOWN_DEG:.0f}° "
