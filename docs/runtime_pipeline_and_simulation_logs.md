...
RealSense RGB + aligned depth
 ...
 -> cuRobo joint-space plan + MoveSplineJoint: overview 복귀
 -> /dsr01/curobo/pick_complete
 -> scan_executor_node가 다음 target 또는 다음 cell 진행
```
...
| overview 이동 | cuRobo joint-space + `MoveSplineJoint` | 안전 거리 확보 후 복귀 |
...
4. 직선 후퇴 완료 후 overview joint pose까지 경로 계획
