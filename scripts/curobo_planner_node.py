...
class CuroboPlanner(Node):
    ...
    def _pick(self, msg: PoseStamped):
        ...
        # 4. 검출된 다른 과실과 하강 corridor가 겹치지 않고 cuRobo endpoint가
        # 유효하면 world -Z로 내려 줄기를 분리한다. 잎/줄기는 world에 없으므로
        # 하강을 보장할 수 없으며, 실행 전 거부 시에만 기존 진입 경로 역주행으로
        # fallback한다. 하강 MoveLine이 시작된 뒤 실패하면 추가 이동을 금지한다.
        downward_status = self._try_downward_detach_retreat(
            straw, grasp_joints, used_grasp_ee_pos, used_grasp_quat
        )
        if downward_status == "failed_after_motion":
            self.get_logger().error(
                "ABORT: downward detach retreat failed after dispatch — "
                "holding current pose; fallback motion blocked")
            self._clear_neighbor_obstacles()
            self._hold_pick_sequence("downward_detach_retreat_failed_after_motion")
            return
        ...
        detach_result = "DETACH_UNVERIFIED"
        detach_reason = (
            "no sensor available; guarded world -Z downward detach executed"
            if downward_status == "executed"
            else "no sensor available; fallback straight reverse retreat succeeded"
        )
        self.runtime_log.log(
            "verify_detach",
            result_code=detach_result,
            grasp_result=grasp_result,
            retreat_policy=downward_status,
