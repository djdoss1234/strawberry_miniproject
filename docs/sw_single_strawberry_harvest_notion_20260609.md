...
## 7. 실제로 딸기를 잡았는지 판단하는 기준

현재 로그의 `grasp OK`는 **그리퍼가 파지 목표 위치까지 이동했다**는 뜻이다.
실제로 줄기를 잡았거나 딸기를 수확했다는 뜻은 아니다.

최종 수확 성공으로 판단하려면 다음 세 단계가 모두 확인되어야 한다.

```text
1. 그리퍼 사이에 줄기가 들어와 실제로 잡힘
2. 딸기가 줄기에서 분리됨
3. 후퇴가 끝날 때까지 딸기를 놓치지 않음
```

### 현재 실제 로그에 남는 값

그리퍼를 닫은 뒤 jaw의 현재 position을 읽어 다음 결과를 기록하도록 구현되어
있다.

| 실제 로그 코드 | 한국어 의미 | 판정 방식 |
| --- | --- | --- |
| `GRASP_CONTACT_DETECTED` | 그리퍼 사이에 무언가 잡힘 | jaw position이 `665`보다 작게 멈춤 |
| `GRASP_EMPTY` | 아무것도 잡지 못하고 완전히 닫힘 | jaw position이 `665` 이상 |
| `GRASP_UNVERIFIED` | 센서값을 읽지 못해 판정 불가 | hardware read 실패 |

최신 SW 실행에서는 다음처럼 기록됐다.

```text
VERIFY_GRASP:
result_code = GRASP_UNVERIFIED
present_position = -1
reason = hardware read failed
```

즉, 최신 실행은 사람이 보기에는 딸기를 잡았지만 로봇 로그만으로는 파지 성공을
자동 확인하지 못한 상태다.

### 아직 실제 로그에 자동 판정되지 않는 값

| 앞으로 확인할 항목 | 의미 | 현재 상태 |
| --- | --- | --- |
| 줄기 분리 성공 | 잡은 딸기가 줄기에서 실제로 떨어졌는가 | 현재 사람 육안 판정 |
| 후퇴 중 유지 성공 | 후퇴가 끝날 때까지 딸기를 놓치지 않았는가 | 측정 필요 |
| 최종 수확 성공 | 잡기 + 분리 + 후퇴 유지가 모두 성공했는가 | 측정 필요 |

### 반복 실험 후 계산할 성공률

| 정량 지표 | 계산 방법 |
| --- | --- |
| 파지 판정 가능 비율 | 센서값을 정상적으로 읽은 횟수 / 전체 close 시도 |
| 실제 파지 성공률 | 줄기를 잡은 횟수 / 전체 파지 시도 |
| 빈 파지 비율 | 아무것도 잡지 못한 횟수 / 전체 파지 시도 |
| 줄기 분리 성공률 | 딸기가 분리된 횟수 / 전체 파지 시도 |
| 후퇴 유지 성공률 | 후퇴 후에도 딸기를 유지한 횟수 / 분리 성공 횟수 |
| 최종 수확 성공률 | 잡기·분리·후퇴 유지가 모두 성공한 횟수 / 전체 시도 |

주의할 점은 jaw position이 중간에서 멈췄더라도 줄기가 아니라 잎을 잡았을 수
있다는 것이다. 따라서 position 값만으로 최종 수확 성공을 선언하지 않고,
다음 place 및 복잡 셀 검증부터 영상 라벨과 함께 기록할 예정이다.
